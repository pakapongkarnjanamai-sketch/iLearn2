...
1. **ก่อนเริ่มงาน:** อ่าน `DOC/AGENT_LOG.md` (10 entry ล่าสุดพอ) เพื่อดูว่า agent อื่นเพิ่งแตะไฟล์ไหน/เปลี่ยน contract อะไร — ห้าม revert งานที่ agent อื่นเพิ่งทำโดยไม่มีเหตุผล ให้ reconcile แทน
   *(ไฟล์เก็บแค่ ~30 entry ล่าสุด ของเก่าอยู่ `DOC/archive/AGENT_LOG-<YYYY-MM>.md` — **ห้าม `Read` ไฟล์ archive ทั้งไฟล์** ใหญ่หลายแสน byte ให้ grep เฉพาะที่ต้องการ)*
2. **หลังจบงานที่มีการแก้โค้ด:** ต่อท้าย entry ใหม่ใน `DOC/AGENT_LOG.md` ตาม format ในไฟล์นั้น (ใหม่สุดอยู่บนสุด) — **ยาวไม่เกิน ~8 บรรทัด** รายละเอียดไปอยู่ในไฟล์แผน ไม่ใช่ใน log
...
5. **การจองเลขแผน:** ใช้ `pwsh tools/plan-status.ps1 -Next` หาเลขถัดไป (สแกนไฟล์จริง **รวม uncommitted ของ agent อื่น**) — ถ้าชนกัน ใครสร้างก่อนได้เลขไป ตัวหลังย้ายเลข (กติกาเต็มใน `DOC/PLANS/README.md`)
6. **หัวไฟล์แผนต้องมีบรรทัด `- **Status:** <STATE>`** (format นี้เป๊ะ ๆ) ไม่งั้น `tools/plan-status.ps1` อ่านสถานะไม่เจอ = แผนหายจากเรดาร์การรีวิว

## เครื่องมือดูแลระบบ agent

| คำสั่ง | ใช้เมื่อ |
|---|---|
| `pwsh tools/plan-status.ps1` | ดูสถานะแผนทั้งหมด + จำนวนหนี้รีวิว + ไฟล์ที่ header เพี้ยน |
| `pwsh tools/plan-status.ps1 -Debt` | รายชื่อแผนที่ implement แล้วแต่ยังไม่ `VERIFIED` (คิวงานของ reviewer) |
| `pwsh tools/plan-status.ps1 -Next` | เลข `PLAN-NNN` ถัดไปที่ว่าง |
| `pwsh tools/rotate-agent-log.ps1` | หมุน `AGENT_LOG.md` เข้า archive เมื่อ entry เกิน ~40 (ลองด้วย `-WhatIf` ก่อนได้) |

กติกาเฉพาะ implementer (Gemini/GPT):

1. รับงานจากไฟล์แผนใน `DOC/PLANS/` ที่สถานะ `READY` และ Assigned ตรงกับตัวเอง — ทำตาม Scope ในแผน **ห้ามขยายขอบเขตเอง** ถ้าเจอปัญหานอกแผนให้จดลงท้ายไฟล์แผน (หัวข้อ Implementer Notes) แล้วทำงานเดิมต่อ
2. ทำเสร็จ: เปลี่ยนสถานะในไฟล์แผนเป็น `DONE` + เติม Implementer Notes (ทำอะไรต่างจากแผนบ้าง/เจออะไร) + รัน verification ตามที่แผนระบุ + ลง AGENT_LOG ตามปกติ

กติกาเฉพาะ reviewer (Claude Code):

1. รีวิวแล้วต้อง **ปิด loop เสมอ** — แก้ Status ในไฟล์แผนเป็น `VERIFIED` (ผ่าน) หรือกลับเป็น `READY` + Reviewer Notes (ไม่ผ่าน). แผนที่ค้าง `DONE` = งานที่ไม่มีใครยืนยันว่าถูก
2. เขียนสถานะเป็น state คำเดียวขึ้นต้นบรรทัดเสมอ (`- **Status:** VERIFIED`) — คำอธิบายยาว ๆ ให้ไปอยู่หัวข้อ Reviewer Notes ท้ายไฟล์ ไม่ใช่ต่อท้ายค่า Status
