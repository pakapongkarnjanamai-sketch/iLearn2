...
- `StatusBadge` / `StatusText` / `CourseStatusBadge` / `CourseStatusText` — thin wrappers over `Badge`; keep using them where they fit. `StatusBadge` derives tone from the status text via `statusTone()` (Completed → green, In Progress/Active/Enrolling → blue, Overdue/Expired → red); `StatusText` is the outlined variant for overview/metadata sections.
...
- `AppButton`, `AppTable`, `AppWizard`, `PageHeader` — existing shared widgets; check here before writing new markup. For async actions pass `loading` to `AppButton` (it shows the spinner + auto-disables) instead of rendering `<Loader2>` by hand.
