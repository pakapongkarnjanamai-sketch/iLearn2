...
- Contract ที่เปลี่ยน: ไม่มี (รีวิว; ยืนยัน `GET MyLearning/Ping` additive ทำงานจริง)
...
- Contract ที่เปลี่ยน (API shape / props / DB): เพิ่ม `GET /MyLearning/Ping` → 204 (additive, ใช้ existing `[Authorize(Policy = "LearnerSession")]`); ไม่เปลี่ยน DB/existing API
- Verified: `dotnet build iLearn.User` → 0 errors, 71 pre-existing warnings; ยังต้อง iPad smoke test (pseudo-fullscreen, pagehide flush, ping keep-alive, numeric keyboard)

## [2026-07-17] GitHub Copilot — PLAN-096 DONE: Learner web performance (compression, cache, asset cleanup)
- ทำอะไร: เพิ่ม Brotli/Gzip response compression ระดับ Fastest ก่อน wwwroot/SCORM static-file middleware; ตั้ง Cache-Control wwwroot 7 วันและ SCORM 1 ชั่วโมงโดยคง nosniff; ตัด ExcelJS/FileSaver/DevExtreme ASP.NET adapters/Font Awesome JavaScript และ global helper ที่ไม่มี caller; เปลี่ยนเป็น jQuery minified; ย้าย toast ขึ้นบนและ lang เป็น th; ลบ dead `baseUrl` เพียงบรรทัดเดียวจาก `Views/MyLearning/Player.cshtml`
- ไฟล์หลักที่แตะ: `iLearn.User/Program.cs`, `iLearn.User/Views/Shared/_DevExtremeLayout.cshtml`, `iLearn.User/Views/MyLearning/Player.cshtml`, `DOC/PLANS/PLAN-096-learner-web-performance.md`, `DOC/AGENT_LOG.md`
- Contract ที่เปลี่ยน (API shape / props / DB): ไม่มี; learner static responses เพิ่ม `Content-Encoding` เมื่อ browser รองรับ และ `Cache-Control` ตามชนิด asset
- Verified: `dotnet build iLearn.User -o artifacts\verify-user` ผ่าน 0 errors (warnings เดิม 71 รายการ); `dotnet build iLearn.Tests -o artifacts\verify-test` ผ่าน 0 errors (warnings เดิม 90 รายการ); `dotnet test artifacts\verify-test\iLearn.Tests.dll` ผ่าน 203/203. ยังต้อง browser/local-header และ QA iPad smoke

## [2026-07-17 —] Claude Code — รีวิว PLAN-096 → REVIEWED ผ่านสะอาด (รอ commit + QA deploy)
- ทำอะไร: ตรวจ diff เต็ม 3 ไฟล์ของ Copilot (Program.cs / _DevExtremeLayout.cshtml / Player.cshtml 1 บรรทัด) — **ตรงสเปคทุกจุด ไม่มี finding** + verify อิสระเกินที่ implementer ทำ: `dotnet test` 203/203 และ**รัน runtime จริง** (dotnet publish → รัน localhost → curl + browser): dx.all.js ตอบ br+Cache-Control 7d → **5.26MB เหลือ 1.98MB (-62%)**, served HTML เหลือ script 5 ตัว/lang=th/ไม่มีซาก API_BASE-ExcelJS-FA JS, toast วัด DOM top=10px, FA icon render ผ่าน webfont ล้วน, console 0 error. Observation เดียว (ไม่บล็อก): br Fastest ได้ 1.98MB สูงกว่าเป้าคาด ~1.6MB — ถูกแล้วเพราะ middleware ไม่มี server-side cache
- ไฟล์หลักที่แตะ: `DOC/PLANS/PLAN-096-*.md` (→REVIEWED + Sign-off), `DOC/AGENT_LOG.md`
- Contract ที่เปลี่ยน: ไม่มี (รีวิว)
- Verified: dotnet test อิสระ + runtime header/DOM proof บน publish จริง; ยังเหลือของที่ต้องดูบน QA: br บน HTTPS จริง (local ทดสอบได้แค่ HTTP), SCORM max-age=3600 (dev ไม่มี D:\iLearnContent), iPad smoke
- หมายเหตุ: **PLAN-097 ถูก mark DONE โดย Gemini ระหว่างที่รีวิว 096 อยู่** — diff 096 ตรวจทันก่อน 097 land จึงสะอาด; Player.cshtml ใน tree ตอนนี้มีงานทั้ง 2 แผนปนกัน ⇒ ควรรีวิว 097 ให้จบก่อนแล้ว commit ทั้งชุดเดียว (แยก commit 094/095 config ที่ค้างออกก่อน) ค่อยให้ Copilot deploy QA

## [2026-07-17 —] Claude Code — เขียน PLAN-096/097: iPad readiness (perf → Copilot, Player UX → Gemini)
- ทำอะไร: ผู้ใช้จะ rollout learner บน iPad → รีวิว readiness ทั้ง iLearn.User แล้วเขียน 2 แผน READY: **PLAN-096 (Copilot)** compression+cache ใน Program.cs (พิสูจน์จาก curl QA: ไม่มี Content-Encoding/Cache-Control — dx.all.js ส่งเต็ม 5.26MB ทุกครั้ง เพราะ in-process hosting ทำให้ IIS static compression ไม่ครอบ) + ตัด asset ตายใน layout ~2.9MB (FA JS ซ้ำ CSS, ExcelJS/FileSaver/aspnet adapters ไม่มี view เรียก, jquery unminified) + toast ขึ้นบน + lang=th. **PLAN-097 (Gemini)** Player บน iPad: flush runtime เพิ่ม pagehide/visibilitychange+sendBeacon (iPadOS ไม่ยิง beforeunload — ผู้ใช้เจอเองว่า progress เสี่ยงหาย), fullscreen เปลี่ยนเป็น pseudo-fullscreen บน touch (ผู้ใช้รายงาน: เลื่อนแล้วหลุดโหมด = system gesture ของ iPadOS แก้ทางอื่นไม่ได้), sidebar กระชับ 400→320px + header ~300→~110px + ปุ่มพับ (ผู้ใช้อนุมัติ mockup), keep-alive ping 10 นาที (คง cookie 30 นาทีเพราะ iPad ใช้ร่วมกัน), touch เก็บตก (inputmode numeric, เลิกพึ่ง unsplash.com ที่เน็ตโรงงานโหลดไม่ได้, :active/44px, gate console.log ของ SCORM adapter, search debounce+แก้บั๊ก highlight)
- ไฟล์หลักที่แตะ: `DOC/PLANS/PLAN-096-learner-web-performance.md` (ใหม่ READY), `DOC/PLANS/PLAN-097-player-ipad-ux.md` (ใหม่ READY), `DOC/AGENT_LOG.md`
- Contract ที่เปลี่ยน: ไม่มีในตัวแผน (097 จะเพิ่ม `GET MyLearning/Ping` → 204 additive; 096 เพิ่ม Content-Encoding/Cache-Control headers)
- Verified: — (แผน; หลักฐานประกอบ: curl QA 3 ครั้งยืนยันไม่บีบอัด, du วัดขนาด asset จริง, grep ยืนยัน createDataStore/handleExporting/API_BASE/baseUrl ไม่ถูกใช้, อ่าน Player/Index/Home/Program.cs เต็มทุกไฟล์)
- **ถึง Copilot + Gemini: ทำคู่ขนานได้ — แบ่งไฟล์ขาดกันแล้ว** (096 = layout+Program.cs เท่านั้น / 097 = Player+Index+Home+MyLearningController; ข้อยกเว้นเดียว: 096 ลบ `const baseUrl` 1 บรรทัดใน Player.cshtml) — ห้ามข้ามเขต; PROD ทั้งคู่รอผู้ใช้ยืนยัน QA บน iPad ในแชทก่อน

## [2026-07-17] GitHub Copilot — PLAN-095 DONE: QA public nikonoa.net SCORM origin + incident documentation
- ทำอะไร: ผู้ใช้ยืนยันว่า iPad เปิด SCORM ผ่าน public PROD URL ได้แล้ว จึงสรุป incident ใน PLAN-094 (หน้าเข้าได้แต่ iframe ได้ internal host จาก `FileSettings:HostUrl`), เปลี่ยน QA `FileSettings:HostUrl` ของ API + Learner เป็น `https://ap-ntc2138-qawb.nikonoa.net/iLearn`, deploy QA API ก่อน Learner, และเพิ่ม checklist ป้องกัน host drift ใน deploy รอบหน้า
- ไฟล์หลักที่แตะ: `iLearn.API/appsettings.json`, `iLearn.User/appsettings.json`, `DOC/PLANS/PLAN-094-public-scorm-content-origin.md`, `DOC/PLANS/PLAN-095-qa-public-scorm-content-origin.md`, `DOC/DEPLOY-CHECKLIST.md`, `DOC/AGENT_LOG.md`
- Contract ที่เปลี่ยน (API shape / props / DB): ไม่มี; QA `launchUrl` เปลี่ยน host จาก internal name เป็น public FQDN เช่นเดียวกับ PROD
- Verified: API/Learner build ไม่มี error; deploy QA API stamp `_deploy_20260717131321` (expected auth health 401, no rollback), QA Learner stamp `_user_deploy_20260717131427` (public health 200, no rollback); read-back QA config ทั้งคู่เป็น public FQDN; public `/iLearn/Courses/` แสดง content folders; ผู้ใช้ยืนยัน iPad PROD launch สำเร็จ

## [2026-07-17] GitHub Copilot — PLAN-094 DONE: public nikonoa.net SCORM launch origin for iPad learner access
- ทำอะไร: ตรวจ public learner URL และ SCORM static asset จริง พบว่า `nikonoa.net/iLearn/Courses/...` ใช้งานได้ แต่ API สร้าง `PlayerContentItemDto.LaunchUrl` ผ่าน `FileSettings:HostUrl` ซึ่ง production ยังชี้ internal host `https://ap-ntc2137-prwb/iLearn`; เปลี่ยน canonical production origin ของ API และ Learner เป็น `https://ap-ntc2137-prwb.nikonoa.net/iLearn` เพื่อให้ iframe SCORM ที่ Player เปิดชี้ public FQDN เดียวกับหน้าเว็บ
- ไฟล์หลักที่แตะ: `iLearn.API/appsettings.Production.json`, `iLearn.User/appsettings.Production.json`, `DOC/PLANS/PLAN-094-public-scorm-content-origin.md`, `DOC/AGENT_LOG.md`
- Contract ที่เปลี่ยน (API shape / props / DB): ไม่มี; ค่า runtime ของ `launchUrl` เปลี่ยน host จาก internal name เป็น public FQDN เท่านั้น
- Verified: build `iLearn.API` + `iLearn.User` ผ่าน (ไม่มี error); deploy PROD API stamp `_deploy_20260717130658` → public health 200, no rollback; deploy PROD Learner stamp `_user_deploy_20260717130840` → public health 200, no rollback; read-back production config ทั้งคู่ชี้ public FQDN; public SCORM entry asset ตอบ content ได้
- คงค้าง: ทดสอบ login และเปิด assigned SCORM บน iPad 1 คอร์ส เพื่อยืนยัน iframe request เป็น public `nikonoa.net/iLearn/Courses/...`
